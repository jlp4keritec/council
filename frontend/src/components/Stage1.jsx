  ...
  return (
    <div className="stage-section">
      <div className="stage-title">
        Étape 1 — Opinions individuelles ({results.length} modèles)
      ...
      {failed.length > 0 && (
        <div className="stage-warning">
          ⚠ <strong>{failed.length} modèle{failed.length > 1 ? 's' : ''} configuré{failed.length > 1 ? 's' : ''} n'a{failed.length > 1 ? 'nt' : ''} pas répondu</strong> :
        ...
        {results.map((r, i) => {
          ...
          return (
            ...
            >
              ...
              {r.from_fallback && (
                <span className="tab-badge tab-badge-fallback" title="Modèle ajouté automatiquement par le fallback pool">
                  ↻
                </span>
              )}
